...
The rest of this paper is organized as follows: section \ref{sec:models} describes the statistical methods and the covariance prior distributions we use. Then in section \ref{sec:results}, we present a simulation study consisting in simulate data from a multivariate normal model and make inference about the covariance matrix to compare the different priors. Finally, in section \ref{sec:birds} to see if what we learned from simulated data is translated on a real data example we estimate the correlation among bird species counts using yearly bird count on Superior national forest. 
...
A similar approach has proposed by \cite{daniels1999} and \cite{matilde}, they use flat priors for the diagonal entries of $\Lambda$ matrix and also let the degrees of freedom parameter have a distribution.  An important advantage of  HIW$_{ht}$ is the implied $\sigma_i \sim t_{\nu}^{+}(0, \xi_i)$ which is recommended by \cite{gelman2006prior}, letting $\xi_i$ have large values we can get weakly informative priors on the variance and maintain the conjugacy of the prior.  
...
Figure \ref{priorF2} shows a scatterplot with the first two standard deviations from 10000 draws of each of the four priors. The IW prior implies a positive relationship among the standard deviations especially in two dimensions. Also large correlations values appear more predominantly when the two variances are high, while low correlations appears when variances are low. 

The SIW and HIW$_{ht}$ priors show a similar picture to the IW prior but the dependence is weaker. For instance, the high correlations are mostly present when standard deviations are also high, however we do see some red points for small values of the standard deviations. Finally,  BMM$_{mu}$ prior is again showing a picture with independence between standard deviations and correlations. 

\subsection{Impact on posterior} 

We study the sensitivity of correlation estimates to the choice of the covariance matrix prior. We simulate mean-zero, normally distributed data in two and ten dimensions with equal variances in all dimensions. Table \ref{scen} shows the sample size, standard deviation, and correlations used in the simulation study. For each combination, 5 replicates were simulated.
...
To reduce Monte Carlo variability in the simulations, data were simulated for $\sigma=1$ and then rescaled to get the others cases with different variances. For instance, there is one data set that consists of ten observations from a bivariate independent standard normal, i.e. $n=10$, $d=2$ $\rho=0$ and $\sigma=1$. Then each simulated value is multiplied by 0.01, 0.1, 10  and 100 to obtain data sets with the relevant standard deviations.

\subsubsection{Inference for Correlation coefficient}

For inference, the true sampling model is assumed and the covariance matrix priors discussed in Section \ref{sec:models} were assumed. Only the bivariate results are shown here but the ten-dimensional results are similar.
...
Figure \ref{devF1} shows a scatterplot of the standard deviation posterior mean against the values used in the simulation. The figure shows that the IW prior overestimates the standard deviation for small true values of the standard deviation.  This bias is attenuated somewhat as sample size increase, but when the true standard deviation is 0.01 the bias is still quite large even with 250 observations. 
...
\section{Bird counts on Superior national forest \label{sec:birds}}

The Natural Resources Research Institute conducts a long-term monitoring program in Superior Natioanl Forest to track regional avian population trends. The specific data set which it is used in this report consist of the yearly bird counts for the 10 most abundant species from 1995 to 2013.  
...
% \end{figure}
The total counts for these 10 most abundant species range from around 100 per year for Least Flycatcher (\emph{Empidonax minimus}) and around 1000 per year for Ovenbirds (\emph{Seiurus aurocapilla}). Since there were around 500 surveys each year, the average count ranged from 0.1 to 2. In all cases, the data are centered within a species by subtracting the average total count (or average count) across the years. 
