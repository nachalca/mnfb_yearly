...
\section{Prior Simulation Results}

<<getsim, message=FALSE, warning=FALSE,include=FALSE, cache=TRUE>>=
res <- read.table('U:\\mnfb_yearly\\data\\reduced_res.csv', header=T)
per <- read.table('U:\\mnfb_yearly\\data\\percentile.csv', header=T)
library(plyr)
library(reshape2)
library(ggplot2)
@
...
qplot(data=per, x=factor(ns), y=prob, geom='boxplot', fill=prior)
qplot(data=subset(per,ns==50) , x=prior, y=per, geom=c('point','boxplot'), fill=prior, facets=s~r)
#qplot(data=subset(per,ns==50), x=prior, y=prob, geom=c('point','boxplot'), fill=prior, facets=s~r)
@
\end{figure}
