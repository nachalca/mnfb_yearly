...
# Stan code for model: y ~ N(mu, Sigma), mu ~ normal(), Sigma ~ P(Sigma)
# the only difference accros models is the covariance matrix prior
...
# SIW model
# Sigma = D*Q*D, diagonal(D) ~ log(normal), Q ~ IW
sim.siw = "
data {
  int <lower=0> N;
  int <lower=0> k;
...
"
#------------------------
# Sigma ~ SS[IG, IW] means Sigma = D * R * D, 
# where D is diagonal with standard deviations that are inverse-gamma distributed
