...
#--------------------------------------------------------
# 4)  DIfferents Betas, Hirerarchical Sigma 
mtext.dh.quad = "
model {
for (i in 1:n) { 
...
# Priors.  
for (j in 1:ns) {
for (i in 1:3) { beta[i,j]   ~ dunif(-1000, 1000) }
...
# Priors.  
for (j in 1:ns) {
for (i in 1:2) { beta[i,j]   ~ dunif(-1000, 1000) }
eta.e[j]      ~ dgamma(alpha,lambda)
sigma.e[j] <- 1/sqrt(eta.e[j])      
}
...
res.dh.q   <- dlply(bird.yeartotal, .(forestN), .fun=runjags, model=mtext.dh.quad, l=3)
res.dh.lin <- dlply(bird.yeartotal, .(forestN), .fun=runjags, model=mtext.dh.lin, l=2)

res.hd.q   <- dlply(bird.yeartotal, .(forestN), .fun=runjags, model=mtext.hd.quad, l=3)
res.hd.lin <- dlply(bird.yeartotal, .(forestN), .fun=runjags, model=mtext.hd.lin, l=2)

res.hh.q   <- dlply(bird.yeartotal, .(forestN), .fun=runjags, model=mtext.hh.quad, l=3)
res.hh.lin <- dlply(bird.yeartotal, .(forestN), .fun=runjags, model=mtext.hh.lin, l=2)
