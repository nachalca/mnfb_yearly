...
# }
# simd2 <- subset(simdata.2, s < 1 )
# simd10 <- subset(simdata.10, s < 11)
...
# prms <- c('s1','s2','rho')
# mod_sciwd2  <-  dlply(simd2, .(s,sim,r,ns), runstan.sim, prm=prms)
# mod_sciwd10 <-  dlply(simd10, .(s,sim,r,ns), runstan.sim, prm=prms)
