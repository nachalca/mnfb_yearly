...
# compute covarince for each model (we should include this into stan models ?)

compucov <- function(res) {
  att <- attributes(res)$split_labels
  res_cov <- NULL
  for (i in 1:dim(att)[1] ) {
    aux <- extract(res[[i]], permuted=FALSE)
    cov <- melt(aux[,,'s1']*aux[,,'s2']*aux[,,'rho'])
    cov <- data.frame(att[i,], t(quantile(cov$value, probs=c(.025, .5, .975))), mean=mean(cov$value) )
...
write.table(res.cov12, file='rescov.csv', row.names=FALSE)
...
# 
# # scale each data set
# simd2  <- ddply(simdata.2, .(s,sim,r,ns) , function(xx) scale(xx[,-c(1:4)], center=FALSE))
# simd10 <- ddply(simdata.10, .(s,sim,r,ns) , function(xx) scale(xx[,-c(1:4)], center=FALSE))
# prms <- c('s1','s2','rho')
# 
